...
def propagate (net : Net Node) (S : Set Node) : Set Node :=
...
--------------------------------------------------------------------
lemma simp_propagate_acc (net : Net Node) :
  n ∉ S
...
--
--------------------------------------------------------------------
lemma activ_agree (net : Net Node) (A B : Set Node) (n : Node) :
  (∀ (m : Node), m ∈ preds net n → (m ∈ A ↔ m ∈ B))
...
--------------------------------------------------------------------
  ...
  case hi L IH =>
    ...
    case mp =>
      ...
      rw [symm hL]
      rw [symm hL] at h₁
      exact @propagate_acc_is_extens Node net _ _ h₁
    ...
    case mpr =>
      ...
      case neg =>
        ...
        case neg =>
          ...
          have h₂ : ∀ (m : Node), m ∈ preds net n → (S₁ m ↔ S₂ m) := by
...
--------------------------------------------------------------------
  ...
  case hi k IH =>
    ...
    case mp =>
      ...
      case pos =>
        rw [symm hL]
        exact @propagate_acc_is_extens Node net _ _ h -- TODO: replace acc variation
      case neg =>
        by_cases n ∈ A
        case pos =>
          rename_i n_not_in_B
        ...
        case neg =>
          ...
          convert h₃ using 4
          rename_i i
          generalize hm : List.get! (net.graph.predecessors n).toList i = m
          generalize hLm : layer net m = Lm
          ...
          have h₃ : m ∈ preds net n := by
            ...
            exact get!_mem (net.graph.predecessors n).toList i
          have h₄ : Lm ≤ k := by
            rw [symm hLm]
            apply Nat.lt_succ.mp
            rw [symm hL]
