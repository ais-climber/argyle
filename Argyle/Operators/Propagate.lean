...
--------------------------------------------------------------------
  ...
  case mp => 
    ...
    case hz => simp [propagate_acc]
    case hi k IH => 
      -- Inductive Step
      intro h₁

      -- By cases; either n ∈ ∅ (trivial), or n ∉ ∅ (n is activated
      -- by m's ∈ ∅)
      by_cases n ∈ (∅ : Set ℕ)
      case pos => exact h
      case neg => 
        ...
        -- Then try to prove what's left.
        simp at h₁
        generalize hm : (fun i => List.get! (net.graph.predecessors n) i) = m
        generalize hLm : (fun i => layer net (m i)) = Lm
      ...
      -- -- Either n ∈ ∅ (trivial), or n was activated by m ∈ ∅.
      -- cases h₁
      -- case inl h₂ => exact h₂
      -- case inr h₂ => 
      --   sorry
  ...
  case mpr => exact fun h₁ => propagate_is_extens _ _ h₁
