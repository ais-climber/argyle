...
-- Convenience rewriting to ensure that the lengths of preceding-weights
-- and preceding-nodes are the same.
same-arity : ∀ {n : ℕ} {graph : Graph ℕ Float n} (node : Fin n)
    → length (preceding-nodes {n} {graph} node) 
        ≡ length (preceding-weights {n} {graph} node)
        
same-arity {n} {graph} node = {!   !}
